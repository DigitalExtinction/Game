...
pub(crate) fn get_keybindings<A: BindableActionlike>(
    ...
    let mut file =
        match std::fs::File::open( conf_dir().expect("Could not get config dir").join(format!("keybindings.{}.ron", action_set_name))) {
            Ok(file) => file,
            Err(_) => {
                std::fs::write(
                    conf_dir().expect("Could not get config dir").join(format!("keybindings.{}.ron", action_set_name)),
