...
use de_signs::{UpdateLineLocationEvent, UpdatePoleLocationEvent};
...
fn change_locations(
    ...
    mut pole_events: EventWriter<UpdatePoleLocationEvent>,
    mut line_events: EventWriter<UpdateLineLocationEvent>,
    transforms: Query<(&Transform, &ObjectType)>,
    solids: SolidObjects,
) {
    for event in events.iter() {
        if let Ok(mut location) = locations.get_mut(event.factory()) {
            ...
            let Ok((transform, object_type)) = transforms.get(event.factory()) else {
                continue;
            };
            let solid = solids.get(*object_type);
            let Some(factory) = solid.factory() else {
                continue;
            };
            let start = transform.transform_point(factory.position().to_msl());
            let end = event.position().to_msl();
            line_events.send(UpdateLineLocationEvent::new(event.factory(), start, end));
        }
    }
}
